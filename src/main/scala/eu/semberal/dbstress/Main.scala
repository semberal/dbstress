...
import eu.semberal.dbstress.model.Results.{
...
object Main extends LazyLogging {
  ...
  def main(args: Array[String]): Unit = {
    val parser = new OptionParser[CmdLineArguments]("dbstress") {
      ...
      opt[JFile]('c', "config")
        .valueName("CONFIG_FILE")
        .text("Path to the configuration YAML file")
        .required()
        .action { (x, c) =>
      ...
      opt[String]('p', "password")
        .valueName("DB_PASSWORD")
        .text("Password to be used for database connections across all units")
        .action { (x, c) =>
      ...
      opt[Unit]('v', "verbose").text("Verbose output").action { (_, c) =>
        LoggerFactory
          .getLogger("eu.semberal.dbstress")
          .asInstanceOf[Logger]
      ...
      case Some(CmdLineArguments(configFile, outputDir, dbPassword)) =>
        parseConfigurationYaml(configFile, dbPassword) match {
          case Right(sc) =>
            ...
            val key =
              "akka.dispatchers.db-dispatcher.thread-pool-executor.core-pool-size-min"
            val config = ConfigFactory
              .load()
              .withValue(key, ConfigValueFactory.fromAnyRef(minConn))

            val system = ActorSystem("dbstressMaster", config)
            val exports: List[ResultsExport] =
              new CsvResultsExport(outputDir) :: Nil
            val future: Future[Unit] = new Orchestrator(system).run(sc, exports)
            val bugMsg =
              "This is most likely an application bug. Please, file an issue in the dbstress bug tracker"
            val exitStatus: Int =
              try {
                ...
                case e: ConnectionInitException =>
                  logger.error(
                    s"Some database connection could not be initialized, terminating the scenario"
                  )
                  3
            ...
            try {
              ...
              case e: TimeoutException =>
                logger.warn(
                  "Unable to shutdown the actor system within the specified time limit",
                  e
                )
              case e: Exception =>
                logger.warn(
                  "Exception during actor system shutdown has occurred",
                  e
                )
            }

...
import eu.semberal.dbstress.model.Configuration.{
...
object UnitSummaryTest {
  ...
  private val unitResult = {
    val dbCommunicationConfig =
      DbCommunicationConfig("A", Some("B"), "C", "D", "E", Some(10))
    val unitRunResults = UnitRunResult(
      DbConnInitResult(1),
      List(
    ...
    UnitResult(
      UnitConfig(
        "unit1",
        Some("This is unit1"),
        UnitRunConfig(dbCommunicationConfig, repeats),
        parallel
      ),
      List(unitRunResults)
    )
